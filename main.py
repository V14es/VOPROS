...
import logging
import re
from ban_users import ban_user, unban_user, check_ban, check_message
...
async def handle_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if check_ban(user_id) != "ban":# and user_id != ADMIN_ID:
        ...
        if check_message(message):
            ...
            ban_user(user_id)
            await update.message.reply_text("Ваш вопрос имеет нежелательное содержание. Вы будете заблокированы")

        else:
            if str(username) == "None":
                save_user(str(user_id), user_id)
                username = user_id
            if find_chat_id_by_username(username) != username:
                save_user(username, user_id)
            print(username)

            if response:
                keyboard = [[InlineKeyboardButton("Не понравился ответ", callback_data="dislike")]]
                reply_markup = InlineKeyboardMarkup(keyboard)
                await update.message.reply_text(text=f"{response}\n\n *ответ подобран автоматически*", parse_mode="Markdown", reply_markup=reply_markup, reply_to_message_id = message_id)
